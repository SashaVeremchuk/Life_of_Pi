...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if (key == GLFW_KEY_W && (action == GLFW_REPEAT || action == GLFW_PRESS)) {
        CamPosition.z += CamSpeed; // Приближение
        CamMoved = true;
    }
    if (key == GLFW_KEY_S && (action == GLFW_REPEAT || action == GLFW_PRESS)) {
        CamPosition.z -= CamSpeed; // Удаление
        CamMoved = true;
    }
    if (key == GLFW_KEY_D && (action == GLFW_REPEAT || action == GLFW_PRESS)) {
        CamPosition.x += CamSpeed; // Движение вправо
        CamMoved = true;
    }
    if (key == GLFW_KEY_A && (action == GLFW_REPEAT || action == GLFW_PRESS)) {
        CamPosition.x -= CamSpeed; // Движение влево
        CamMoved = true;
    }

    // Вы можете добавить условие для высоты, если это необходимо
    // if (key == GLFW_KEY_SPACE && (action == GLFW_REPEAT || action == GLFW_PRESS)) {
    //     CamPosition.y += CamSpeed; // Движение вверх, если это нужно
    //     CamMoved = true;
    // }
    // if (key == GLFW_KEY_LEFT_SHIFT && (action == GLFW_REPEAT || action == GLFW_PRESS)) {
    //     CamPosition.y -= CamSpeed; // Движение вниз, если это нужно
    //     CamMoved = true;
    // }*/
}
